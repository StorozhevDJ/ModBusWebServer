...
public class DeviceInfoModbusMapMapper {
	 ...
	 */
	public void ParseRegisterDevInfo(Register[] slaveResponse) {
		// setId(this.id);
		setType(slaveResponse[modBusRegMap.DEV_ID.ordinal()].toShort()); // ro Идентификационный код прибора (default
																			// 150)
		hw_ver = slaveResponse[modBusRegMap.HW_Version.ordinal()].toShort(); // ro Модификация прибора
		setDev_serial(slaveResponse[modBusRegMap.DEV_Serial.ordinal()].toShort());// ro Серийный номер прибора
		mfg_year = slaveResponse[modBusRegMap.MFG_Year.ordinal()].toShort(); // ro Год выпуска прибора
		fw_ver = slaveResponse[modBusRegMap.FW_Version.ordinal()].toShort(); // ro Версия прошивки прибора (Firmware
																				// Version = REG/10 )
		min_voltage = slaveResponse[modBusRegMap.MinVoltage.ordinal()].toShort();// ro Минимальное напряжение питания
																					// UMIN = REG/100, V (default=550)
		max_voltage = slaveResponse[modBusRegMap.MaxVoltage.ordinal()].toShort();// ro Максимальное напряжение питания
																					// UMAX = REG/100, V (default=2400)
		current = slaveResponse[modBusRegMap.Current.ordinal()].toShort(); // ro Ток потребления INOM = REG/100, mA
																			// (default=300)
		baudrate = slaveResponse[modBusRegMap.Baudrate.ordinal()].toShort(); // ro Поддерживаемые скорости обмена
	...
	public void setComment(Register[] reg) {
		...
		try {
			String string = new String(strData, "Cp1251");
	...
	public DeviceInfo mapDeviceInfo(Register[] regs) {
		ParseRegisterDevInfo(regs);
		...
		DeviceInfo dev = new DeviceInfo(0, getSubAddress(), typeStr, getDev_serial(), getComment());
		return dev;
	}
